function [p,stats] = scatterbars(Y,type,xoffset,ytext,dispName)
arguments
    Y cell
    type (1,:) char = 'basic'
    xoffset double = 0
...
[~,idx] = min(abs(xi'-Y{i}'));
width = f(idx)/max(f)
xposjit{i} = jitter(xpos,3.5*width');

    scatter(xpos(1),mu,70,'o','k','filled','HandleVisibility','off');
        hold on

    eb = errorbar(xpos(1),mu,sig,'color','k','LineWidth',3,'DisplayName',dispName{i}); 
    scatter(xposjit{i},Y{i},30,'k','filled','MarkerFaceAlpha',0.15,...
